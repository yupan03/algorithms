...
public class Graph {
    ...
    public void addEdge(String startLab, String endLab) {
        int start = getVertex(startLab);
        int end = getVertex(endLab);
        if (start == -1 || end == -1) {
    ...
    private int getVertex(String lab) {
        for (int i = 0; i < vertexList.length; i++) {
            Vertex vertex = vertexList[i];
            if (vertex.label.equals(lab)) {
                return i;
            }
        }
        return -1;
    ...
    /**
     * 深度优先搜索算法: 1、用peek()方法检查栈顶的顶点 2、用getAdjUnvisitedVertex()方法找到当前栈顶点邻接且未被访问的顶点
    ...
    /**
     * 广度优先搜索算法： 1、用remove()方法检查栈顶的顶点 2、试图找到这个顶点还未访问的邻节点 3、如果没有找到，该顶点出列
